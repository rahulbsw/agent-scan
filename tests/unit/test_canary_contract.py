...
primitives: it calls ``scope.commands(ctx)`` and runs each :class:`~agent_scan.canary_test_supported_agents.base.SeedCommand` as
a subprocess (reading ``argv`` / ``run_in_project`` / ``timeout`` / ``non_fatal``), and it calls
``scope.files()`` and copies each :class:`~agent_scan.canary_test_supported_agents.base.FixtureFile` into the project (reading
``src`` / ``dest``). This test pins that contract from the agent-scan side — every concrete
:class:`~agent_scan.canary_test_supported_agents.base.Scope` subclass must be exercised by a real canary and must emit commands
...
from agent_scan.canary_test_supported_agents import CANARIES, CanaryContext, FixtureFile, Scope, SeedCommand
...
def _concrete_scope_subclasses() -> set[type[Scope]]:
    """Every loaded concrete subclass of ``Scope`` (importing ``agent_scan.canary_test_supported_agents`` registers them all)."""
