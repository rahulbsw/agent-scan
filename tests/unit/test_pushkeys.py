...
class TestFetchGuardEnabled:
    def test_returns_true_when_api_enables(self):
        ...
        with patch("agent_scan.pushkeys.urlopen", return_value=mock_cm) as mock_urlopen:
            assert fetch_guard_enabled("https://hooks.example.com", "tid", "tok") is True

        request_obj = mock_urlopen.call_args[0][0]
        assert request_obj.get_full_url() == _build_guard_enabled_url("https://hooks.example.com", "tid")
    ...
    def test_returns_false_when_api_disables(self):
        ...
        with patch("agent_scan.pushkeys.urlopen", return_value=mock_cm):
            assert fetch_guard_enabled("https://hooks.example.com", "tid", "tok") is False
    ...
    def test_raises_on_bad_json_shape(self):
        ...
        with patch("agent_scan.pushkeys.urlopen", return_value=mock_cm):
            with pytest.raises(RuntimeError, match="Unexpected guard-enabled"):
                fetch_guard_enabled("https://hooks.example.com", "tid", "tok")

    def test_raises_access_denied_on_403(self):
        url = _build_guard_enabled_url("https://hooks.example.com", "tid")
...
class TestMintPushKeyUrl:
    """Smoke test for shared URL normalization."""

    def test_builds_agent_scan_tenants_path(self):
        assert _build_guard_enabled_url("https://hooks.example.com", "tid-1") == (
            f"https://hooks.example.com/tenants/tid-1/agent-scan/hooks-enabled?version={HOOK_VERSION}"
        )
