...
class AgentDiscoverer(ABC):
    ...
    def _iter_plugin_mcp_files(self, bases: list[Path], filenames: tuple[str, ...]) -> Iterator[Path]:
        """Yield each plugin MCP file named in ``filenames`` under the base dirs.

        The ``is_file()`` guard skips FIFOs/special files ``os.walk`` may surface
        (a deliberate hang defense).
        """
        for base in bases:
    ...
    def _discover_plugin_mcp_files(
        ...
        (:meth:`_iter_plugin_mcp_files`). A falsy result (``None`` or empty list) is
        skipped; a truthy ``CouldNotParseMCPConfig`` is recorded.
        """
        result: McpConfigsResult = {}
        for mcp_file in self._iter_plugin_mcp_files(bases, filenames):
